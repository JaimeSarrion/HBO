@extends('navbar')
@section('form')
@parent

<div class="_2PURe u2Omg _1JVKc" style="opacity: 1;">
<div class="_2-zGO">
   <div data-automation="account-state-error-banner" class="jI8J8 _3_YIA" tabindex="0">
      <div class="_1xjXq">
         <div aria-hidden="true" class="E3e2X _1UAj4">
            <svg width="15" height="15" viewBox="0 0 15 15" xmlns="http://www.w3.org/2000/svg">
               <title>CBFC85DD-8509-4F77-94BB-2FC8137009FB</title>
               <g id="Symbols" fill="none" fill-rule="evenodd">
                  <g id="icon-/-adult-/-misc-/-alert" fill="#FFF">
                     <path d="M7.5 0a7.5 7.5 0 1 0 0 15 7.5 7.5 0 0 0 0-15zm1.201 11.1c0 .608-.468 1.076-1.1 1.076-.633 0-1.101-.468-1.101-1.075v-.026c0-.607.468-1.075 1.1-1.075.633 0 1.101.468 1.101 1.075v.026zM6.5 4.1a1.1 1.1 0 1 1 2.2 0v3.8a1.1 1.1 0 1 1-2.2 0V4.1z" id="Shape"></path>
                  </g>
               </g>
            </svg>
         </div>
         <div class="OboPp">Reactiva tu suscripción hoy y vuelve a tener acceso a las mejores series online.</div>
      </div>
      <div class="OGkKw"><a data-analytics="activation-notification" aria-labelledby="Reactivar" href="https://es.hboespana.com/my-account/subscription" class="_3Rqbb">Reactivar</a></div>
   </div>
</div>
<div class="pull-below-header">
   <div class="_1zJio">
      <div class="_35agv _3vT_z">
         <div class="thumbnail-image rTzfU" role="img" aria-label="Juego de tronos" tabindex="-1" style="opacity: 1; image-rendering: -webkit-optimize-contrast; background-image: url(&quot;{{$serie -> imagen}}&quot;);"></div>
      </div>
   </div>
   <!-- react-empty: 3438 -->
   <div class="GnMTX">
      <div class="_2KlrJ _8tZJP et3Mg">
         <div class="_1fzpM"><span class="_1KTsC _2R_OM" tabindex="0" aria-label="Calificación:" style="background-color: rgb(255, 0, 0); color: rgb(255, 255, 255);">18</span><span tabindex="0" class="azoai _2WKMQ">{{$serie -> genero}}</span></div>
         <div class="_2xwoD">
            <h1 tabindex="0" class="_1YoH- LD_c1">{{$serie -> nombre}}</h1>
         </div>
         <div class="_1N6rE"></div>
      </div>
      <div class="v9Wel _8tZJP et3Mg">
         <!-- react-empty: 3494 -->
         <nav class="FjPf5 UMVMU _311Q5">
            <div class="_3vAzj">
               <div class="_1lWob">
                  <ul class="_3d8o9">
                     <li  class="_1KV1E active"><a href="/show/series/{{$serie->id}}" class="cz0LH"><span class="_3iRv9">Sinopsis </span></a></li>
                     @foreach($temporadas as $temp)
                     <li class="_1KV1E"><a href="/show/series/{{$serie->id}}/temporada/{{$temp->temporada}}" class="cz0LH"><span class="_3iRv9">Temporada {{$temp->temporada}} </span></a></li>
                     @endforeach
                     <li class="ze722">
                        <div class="_2gScx" style="transform: translate(15px); width: 73px;"></div>
                     </li>
                  </ul>
               </div>
            </div>
         </nav>
      </div>
      
      <div class="_2wUoj DP5C6">
         <div class="_2Sx8A">
             @if(Auth::user())
            <div class="_1-u7n" tabindex="0">
               <div tabindex="0" class="_2-oIH _3M_lL bGzAP">Comenzar a ver</div>
               <figure class="_1OPcT _1vbnE _2JbnS">
                  <div class="preview _1v20p">
                     <div class="_3vT_z">
                        <div class="thumbnail-image rTzfU" role="img" aria-label="Se acerca el invierno" tabindex="-1" style="opacity: 1; background-image: url(&quot;https://production-static.hboespana.com/p/3b1f95c-009b0a2672b/HBOE-AADGT-000-PGM-01-01-2500-HD-169-SR-1920x1080-50000-10228178.jpg&quot;); image-rendering: -webkit-optimize-contrast;"></div>
                     </div>
                     <div class="_1F2HS qK-aZ">
                        <div class="_88XGK _3LXT0"></div>
                        <div class="AQT1f _2ZMRQ">
                           <button type="button" aria-label="Añadir a Mi Lista" class="_2Uyxi _26mhH" data-automation="toggle-watchlist-button" data-tooltip="Añadir a Mi Lista" data-tooltip-class="_2zgDa _3YdbR">
                              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 30 30" stroke="currentColor" stroke-linecap="round">
                                 <circle class="circle" cx="15" cy="15" r="13.5" fill="none"></circle>
                                 <path class="line horizontal" d="M8 15h14"></path>
                                 <path class="line vertical" d="M15 8v14"></path>
                              </svg>
                           </button>
                        </div>
                        <div class="f0RPe">
                           <a href="https://es.hboespana.com/series/juego-de-tronos/season-1/episode-1/3b1f95c-009b0a2672b/play" class="Xlbkc TKCzG" data-automation="play-button">
                              <svg width="20" height="26" viewBox="0 0 20 26" xmlns="http://www.w3.org/2000/svg" aria-label="Haz clic para reproducir el contenido" class="_27Bb4">
                                 <title>A9499208-7E03-41C9-B521-6AF078D61FE2</title>
                                 <g id="icon-play-symbols" fill="none" fill-rule="evenodd">
                                    <g id="icon-play" transform="translate(-25 -20)">
                                       <path d="M45.5 22.106c0-1.106-.76-1.536-1.701-.958L26.197 31.956c-.94.577-.94 1.511 0 2.09L43.8 44.852c.94.577 1.701.157 1.701-.958V22.106z" id="icon-play-path-2" fill="currentColor" transform="matrix(-1 0 0 1 70.5 0)"></path>
                                    </g>
                                 </g>
                              </svg>
                           </a>
                        </div>
                     </div>
                     <a href="https://es.hboespana.com/series/juego-de-tronos/season-1/episode-1/3b1f95c-009b0a2672b" class="link-primary needsclick c_VCX" aria-hidden="true" tabindex="-1"></a>
                  </div>
                  <figcaption class="caption">
                     <a href="https://es.hboespana.com/series/juego-de-tronos/season-1/episode-1/3b1f95c-009b0a2672b" class="link-primary needsclick ac_ai _1hvmS _3bOBw" aria-label="Se acerca el invierno">
                        <span class="_3HhQi _3icqI ea_Q3">Se acerca el invierno</span>
                        <div class="_34PHF _1N1Iu"><span>T1 / E1</span></div>
                     </a>
                  </figcaption>
               </figure>
            </div>
            @endif
            <div tabindex="0" class="_1GVJl">
               <div class="_3wBBp">
                  <p class="_1l7DL" data-automation="synopsis">{{$serie -> sinopsis}}</p>
               </div>
               <dl class="_1sfUQ">
                  <div data-automation="meta_actor" class="_1vAb9">
                     <dt class="_2Cex9">Reparto</dt>
                     <dd class="_38rx6">{{$serie -> reparto}}</dd>
                  </div>
                  <div data-automation="meta_creator" class="_1vAb9">
                     <dt class="_2Cex9">Creadores</dt>
                     <dd class="_38rx6">{{$serie -> creador}}</dd>
                  </div>
                  <div data-automation="meta_year" class="_1vAb9">
                     <dt class="_2Cex9">Año</dt>
                     <dd class="_38rx6">{{$serie -> anyo}}</dd>
                  </div>
               </dl>
            </div>
         </div>
      </div>
   </div>
</div>
</div>
<div class="_2SaYk"></div>
@endsection